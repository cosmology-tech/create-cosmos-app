...
export function SwapTokenInput({
  ...
  function onItemSelected(item: ComboboxOption) {
    onTokenSelected(
      tokens.find(({ symbol }) => symbol === item.tokenName) as Token
    );
  }
  ...
  const addon = (
    <Stack direction="vertical" space="$0">
      {disabled ? (
        <Text fontSize="$2xl" textAlign="right" fontFamily="monospace">
          {amount}
        </Text>
      ) : (
        <input
          ...
          style={{
            ...
            color:
              colorMode === 'dark' ? 'rgb(238, 242, 248)' : 'rgb(44, 49, 55)',
            border: 0,
            outline: 'none',
            appearance: 'none',
            fontSize: '22px',
  ...
  return (
    <Stack
      ...
      attributes={{
        ...
        borderColor: '$divider',
        borderWidth: colorMode === 'light' ? '1px' : '$none',
        borderStyle: 'solid',
      }}
    >
      <Stack
      ...
      >
        <Text color="$textSecondary" attributes={{ minWidth: '66px' }}>
          {title}
        </Text>
        ...
        <Stack space="$5" attributes={{ alignItems: 'center' }}>
          ...
          <Stack
            space="$5"
            attributes={{ flex: 1, justifyContent: 'flex-end' }}
          >
            {showHalfButton ? (
              <AmountButton onClick={onHalfButtonClick}>Half</AmountButton>
            ) : null}
            {showMaxButton ? (
              <AmountButton onClick={onMaxButtonClick}>Max</AmountButton>
            ) : null}
          </Stack>
        </Stack>
      </Stack>
