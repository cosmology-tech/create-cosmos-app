...
import { Token, Slippages, TokenList } from "@/components/swap";
import { SwapInfoProps } from "@/components/swap/SwapInfo";
...
export function newTokens(
  ...
  tokens.hash = tokens.reduce((acc, token) => ({ ...acc, [token.denom]: token }), {}) as Record<CoinDenom, Token>;
