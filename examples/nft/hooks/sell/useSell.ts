...
export function useSell() {
  ...
  const name =
    names && names.length > 0 ? names[0].name + '.stars' : shorten(address);

  const isVerified = Boolean(
    names?.[0]?.records?.some((record) => record.verified)
  );
  ...
  const meta = [
    ...
    },
    {
      ...
      value: String(
        tokens?.tokens.filter((token) => token.forSale).length || 0
      ),
    },
  ];
  ...
  const onClickToken = (i: number) => {
    ...
    setCollection(
      collections?.collections.find(
        (collection) =>
          collection.collectionAddr === tokens?.tokens[i]?.collectionAddr
      )!
    );
    openModal();
  };

  const list: NftProfileCardProps[] =
